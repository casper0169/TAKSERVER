...
def gestionar_usuarios():
    ...
    if opcion == "1":
        ...
        if os.path.isfile(f"/opt/tak/certs/files/{certificado}.pem"):
            ...
            if subopcion == "1":
                ejecutar_comando(f"sudo java -jar /opt/tak/utils/UserManager.jar certmod -A /opt/tak/certs/files/{certificado}.pem")
                ...
                if tipo_grupo == "Entrada":
                    ...
                    ejecutar_comando(f"sudo java -jar /opt/tak/utils/UserManager.jar certmod -ig{grupo} /opt/tak/certs/files/{certificado}.pem")
                elif tipo_grupo == "Salida":
                    grupo = input("Indique el nombre del grupo: ")
                    ejecutar_comando(f"sudo java -jar /opt/tak/utils/UserManager.jar certmod -og{grupo} /opt/tak/certs/files/{certificado}.pem")
                elif tipo_grupo == "Ambas":
                    grupo = input("Indique el nombre del grupo: ")
                    ejecutar_comando(f"sudo java -jar /opt/tak/utils/UserManager.jar certmod -g{grupo} /opt/tak/certs/files/{certificado}.pem")
            else:
                print("Opción inválida.")
        else:
        ...
        certificado = input("Ingrese el nombre del certificado: ")
        if os.path.isfile(f"/opt/tak/certs/files/{certificado}.pem"):
            ejecutar_comando(f"sudo java -jar /opt/tak/utils/UserManager.jar certmod -r /opt/tak/certs/files/{certificado}.pem")
...
def mostrar_urls():
    ...
    print("TAK Server Admin Tool: http://localhost:8080")
    print("Servidor Web TAK: http://localhost:8443")
